...
export const PromptComponencts = React.memo(function ({ imageSetting, state, canvasOrientation, fabricJs }: ImageSettingProps) {
    ...
    async function wholeCanvasReference() {
        try {
            ...
            const file = new File([data], "wholerefrenceImage.png", { type: data.type || "image/png" });
            ...
            setUploadedWholeCanvasData((prev) => ({ ...prev, uploaded: false, ...fileData }))
            ...
            const erorObject = error as { message: string }
            setError({
                type: "error",
                message: erorObject.message as string
            })

        } finally {
            setUploadedWholeCanvasData((prev) => ({ ...prev, uploaded: false }))
        }
    ...
    const refinePrompt = async () => {
        try {
            ...
            if (!loginUserData?.id) {
                setError({
                    type: "error",
                    message: "Please login for using this feature"
                })
            ...
            if (!usingApi) {
                // console.log("api calling going to the backend not present any apikey so.........");
                const responseJson = await ApiEndpoint.Post("ai-prompt-refine", {}, { content, id: loginUserData?.id, systemPromptSelected });
                aiPromptRefine = responseJson.data
                ...
                aiPromptRefine = await ai.models.generateContent({
                                            ...
                                            {uploadedWholeCanvasData.uri.trim().length > 0 ?
                                                ...
                                                <span className='flex items-center justify-center gap-0.5'>
                                                    ...
                                                    <CloudUpload size={22}
                                                        ...
                                                        //     getBoundingBox(e, "Gemina Upload", "top")
                                                        // }}
                                                        onClick={
                                                            async () => {
                                                                // setPortalElement(null)
