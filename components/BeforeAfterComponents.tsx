    ...
    <AnimatedSection className='mx-auto mt-20 w-full max-w-7xl rounded-3xl border border-white/5 bg-[#121212]/80 backdrop-blur-sm px-4 py-12 sm:px-8 lg:px-12 shadow-2xl overflow-hidden hover:border-white/10 transition-colors'>
      <div className='flex flex-col gap-10 lg:flex-row lg:items-center lg:justify-between'>
        ...
        <div className='flex min-w-0 flex-1 flex-col z-10'>
          ...
          <Link href='/image-home-screen' className='w-fit rounded-xl bg-white px-8 py-3.5 text-sm font-bold text-black transition-all hover:bg-gray-200 shadow-[0_0_20px_rgba(255,255,255,0.3)] mb-8'>
            Try Image AI Now
          </Link>
          ...
          <div className='w-full'>
            ...
            <div className='copyingSectionScrollbar flex w-full gap-4 overflow-x-auto pb-4'>
              {Array.from({ length: 1 }).map((_, index) => (
                <CopyBox key={index} copyRef={copyingRefELement} copying={copying} copyToClipboard={copyToClipboard} index={index} text={copyTags} className='bg-white/[0.03] border-white/10 hover:bg-white/[0.06] transition-colors rounded-2xl p-4 w-full' />
              ))}
            </div>
          </div>
        </div>
